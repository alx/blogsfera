...
function prepare_query() {
  ...
  switch($this->type) {
    ...
    // ============== get_feed api call
    case 'get_feed':
    $get_feed['feed_url']=(!isset($this->params['feed_url'])) ? '' : "&feed_url=".$this->params['feed_url'];
    // == Prepare the url for get_feed api query
    $path   = "/api/get_feed";
    $query  = "api_key={$this->api_key}{$get_feed['feed_url']}";
    break;

    // ============== add_feed api call
    case 'add_feed':
    $add_feed['url']=(!isset($this->params['url'])) ? '' : "&url=".$this->params['url'];
    $add_feed['href']=(!isset($this->params['href'])) ? '' : "&href=".$this->params['href'];
    // == Prepare the url for add_feed api query
    $path   = "/api/add_feed";
    $query  = "api_key={$this->api_key}{$add_feed['url']}{$add_feed['href']}";
    break;
    ...
    // ============== delete_feed api call
    case 'delete_feed':
    $delete_feeds['feeds_id']=(!isset($this->params['feed_id'])) ? '' : "&feeds_id=".$this->params['feed_id'];
    // == Prepare the url for delete_feeds api query
    $path   = "/api/delete_feeds";
    $query  = "api_key={$this->api_key}{$delete_feeds['feeds_id']}";
    break;
    ...
    // ============== edit_tags api call
    case 'edit_tags':
    $edit_tags['feed_url']=(!isset($this->params['feed_url'])) ? '' : "&feed_url=".$this->params['feed_url'];
    $edit_tags['tag_list']=(!isset($this->params['tag_list'])) ? '' : "&tag_list=".$this->params['tag_list'];
    // == Prepare the url for edit_tags api query
    $path   = "/api/edit_tags";
    $query  = "api_key={$this->api_key}{$edit_tags['feed_url']}{$edit_tags['tag_list']}";
    break;

    // ============== edit_avatar api call
    case 'edit_avatar':
    $edit_avatar['feed_url']=(!isset($this->params['feed_url'])) ? '' : "&feed_url=".$this->params['feed_url'];
    ...
    $query  = "api_key={$this->api_key}{$edit_avatar['feed_url']}{$edit_avatar['avatar_url']}";
    break;
...
function get_content() {
  ...
  switch($this->type) {
    ...
    if(isset($xml_array['feevy']) && is_array($xml_array['feevy'])) {
      ...
      $result['error'] = "An error occured while retrieving feed with {$this->api_key}";
    }
    break;
    ...
    default:
      $result['error'] = "An error occured while fetching API type";
    break;
  }
