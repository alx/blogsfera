...
************************************************************************************************************/
	...
	*/	
	...
	function startMove()
	{
		if (typeof (moveCounter) == "undefined") return;
	...
	function hideWindow()
	{
		hideWindowByID('dhtml_goodies_id' + this.id.replace(/[^\d]/g,''));
	}
	
	function hideWindowByID(id)
	{
		switchElement(false,document.getElementById(id));	
		activeWindow.style.display='none';
		var parent = activeWindow.parentNode;
		parent.removeChild(activeWindow);
	}

	/**
	 * Closes the window the specified element is contained in.
	 */
	function closeParentWindow(element)
	{
		while (element)
		{
		
			if (element.id.indexOf('dhtml_goodies_id') != -1)
			{
				hideWindowByID(element.id);
				break;
			}
	...
	function createNewWindow(width,height,left,top)
	{
		...
		{
			document.getElementById(conf.root).appendChild(div);
		}
		...
		img.src = conf == null ? 'images/top_left.gif' : conf.img_top_left;
		img.align='left';
		topDiv.appendChild(img);
		
		var img = document.createElement('IMG');
		img.src = conf == null ? 'images/top_center.gif' : conf.img_top_center;
		...
		var img = document.createElement('IMG');
		img.src = conf == null ? 'images/minimize.gif' : conf.img_minimize;
		img.className='minimizeButton';
		buttonDiv.appendChild(img);	
		
		var img = document.createElement('IMG');
		img.src = conf == null ? 'images/close.gif' : conf.img_close;
		img.className='closeButton';
		buttonDiv.appendChild(img);	
		
		var img = document.createElement('IMG');
		img.src = conf == null ? 'images/top_right.gif' : conf.img_top_right;
		buttonDiv.appendChild(img);		
		...
		var img = document.createElement('IMG');
		img.src = conf == null ? 'images/bottom_right.gif' : conf.img_buttom_right;
